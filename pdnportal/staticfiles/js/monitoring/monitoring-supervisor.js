...
function updateOutputChart(period, isAutoRefresh = false) {
  ...
  fetch(`/monitoring/chart-data/${period}/`, {
      ...
      .catch(error => {
          ...
          // Update chart with empty data to avoid displaying stale data
          if (outputChart) {
              outputChart.data.labels = [];
              outputChart.data.datasets[0].data = [];
              outputChart.data.datasets[1].data = [];
              outputChart.update();
          }
      });
}
...
function setupEventListeners() {
    ...
    if (addProductForm) {
        addProductForm.addEventListener('submit', function(e) {
            ...
            const groupId = formData.get('monitoring_id');
            ...
            .catch(error => {
                ...
                // If JSON parsing fails, just return a success message
                return { status: 'success', message: 'Product added successfully!' };
            })
            .then(data => {
                if (data.status === 'success' || !data.status) {
                    createToast(data.message || 'Product added successfully!', 'success');

                    // Reset the form
                    this.reset();

                    // Refresh the products table without closing the modal
                    if (groupId) {
                        fetch(`/monitoring/get-group/${groupId}/`)
                            .then(response => response.json())
                            .then(groupData => {
                                if (groupData.products) {
                                    updateProductsTable(groupData.products);
                            ...
                            .catch(error => {
                                ...
                                // Reset the submit button state on error too
                                const submitBtn = document.getElementById('save-product');
                                if (submitBtn && submitBtn.classList.contains('loading')) {
                                    submitBtn.classList.remove('loading');
                                    submitBtn.textContent = 'Add Product';
                    ...
                    createToast(data.message || 'Failed to add product', 'error');
    ...
    if (addScheduleForm) {
        addScheduleForm.addEventListener('submit', function(e) {
            ...
            // Show loading state on submit button
            const submitBtn = document.getElementById('save-schedule');
            if (submitBtn) {
                submitBtn.classList.add('loading');
                submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
            }
            ...
            .then(data => {
                if (data.status === 'success' || !data.status) {
                    createToast(data.message || 'Schedule added successfully!', 'success');

                    // Reset the form
                    this.reset();

                    // Refresh the schedules table without closing the modal
                    if (groupId) {
                        fetch(`/monitoring/get-group/${groupId}/`)
                            .then(response => response.json())
                            .then(groupData => {
                                if (groupData.schedules) {
                                    updateSchedulesTable(groupData.schedules);
                    ...
                    createToast(data.message || 'Failed to add schedule', 'error');
...
function setupGroupSearch() {
    ...
    const groupsContainer = document.querySelector('.PM-groups-container');

    if (!searchInput || !filterSelect || !groupsContainer) return;

    // Function to update empty state display
    function updateEmptyState(shouldShow, isSearching) {
        let emptyState = groupsContainer.querySelector('.PM-empty-state');
        ...
        if (shouldShow) {
            ...
            const message = isSearching ?
                'No matching Monitoring Groups found' :
                'No Monitoring Groups available';

            const description = isSearching ?
                'Try adjusting your search criteria' :
                'Create a new monitoring group to get started';

            emptyState.innerHTML = `
                <div class="PM-empty-icon">
                    <i class="fas fa-${isSearching ? 'search' : 'users-cog'}"></i>
                </div>
                <h3>${message}</h3>
                <p>${description}</p>
            `;
    ...
    function filterGroups() {
        ...
        groupCards.forEach(card => {
            ...
            const lines = Array.from(card.querySelectorAll('.PM-line-tag'))
                .map(line => line.textContent.toLowerCase());

            const matchesSearch = title.includes(searchTerm) ||
                                lines.some(line => line.includes(searchTerm));
