...
def get_monitoring_group(request, group_id):
    ...
    products_data = []
    for product in monitoring.monitoring_product.all():
    ...
    schedules_data = []
    for schedule in ProductionSchedulePlan.objects.filter(monitoring=monitoring).order_by('-date_planned'):
...
def add_product(request):
    ...
    # If form is invalid, return to dashboard with error messages
...
def get_chart_data(request, period):
    ...
    print(f"Chart data for period {period}: {response_data}")  # Add this debug line
