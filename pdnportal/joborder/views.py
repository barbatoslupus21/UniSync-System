...
def requestor_page(request):
    ...
    pendingJO = JOLogsheet.objects.filter(Q(status="Routing") | Q(status="Completed"), prepared_by=request.user).order_by("-date_created")
...
def create_jo_request(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if not all([category, tooling, nature, details]):
                missing = []
...
def job_order_facilitator(request):
    ...
    pendingJOCount = JORouting.objects.filter(jo_number__in_charge__isnull=False, approver_sequence=5, status = "Pending", approver=request.user, request_at__year=current_year, request_at__month=current_month).count()
...
def job_order_analytics(request):
    ...
    for i in range(months):
        ...
        month_end = today - timedelta(days=30 * (months - i - 1))
...
def job_order_stats_api(request):
    try:
        ...
        # Active JO count in last 30 days
        active_jo_count = JOLogsheet.objects.filter(
            in_charge__in=maintenance_users,
            status__in=['Routing', 'Completed', 'Checked'],
            date_created__gte=thirty_days_ago
        ).count()

        # Previous 30-day active JO count
        prev_active_jo_count = JOLogsheet.objects.filter(
            in_charge__in=maintenance_users,
            status__in=['Routing', 'Completed', 'Checked'],
            date_created__range=[sixty_days_ago, thirty_days_ago]
        ).count()

        active_jo_percentage = (
            round(((active_jo_count - prev_active_jo_count) / prev_active_jo_count) * 100)
            if prev_active_jo_count > 0 else 100
        )
        ...
        completion_rate = (
            round((completed_jo_last_30_days / total_jo_last_30_days) * 100)
        ...
        # Overdue tasks
        overdue_tasks = JOLogsheet.objects.filter(
            status__in=['Routing'],
            target_date__lt=now
        ).count()

        prev_overdue_tasks = JOLogsheet.objects.filter(
            status__in=['Routing'],
            target_date__range=[sixty_days_ago, thirty_days_ago],
            target_date__lt=thirty_days_ago
        ).count()
...
def job_order_timeline_api(request, view_type):
    try:
        ...
        if view_type == 'timeline-today':
            ...
            today_jobs = JORouting.objects.filter(
            ...
            for job in today_jobs:
                response_data['events'].append({
                    ...
                    'time': job.request_at.strftime("%I:%M %p"),
            ...
            week_jobs = JORouting.objects.filter(
                approver__in=maintenance,
                status__in=['Pending', 'Approved'],
                request_at__range=[week_start, week_end]
            ).order_by('request_at')
            ...
            for job in week_jobs:
                day_column = day_columns.get(job.request_at.day, 1)
                response_data['events'].append({
                    'jo_number': job.jo_number.jo_number if hasattr(job.jo_number, 'jo_number') else str(job.jo_number),
                    'time': job.request_at.strftime("%I:%M %p"),
                    'title': f"Assigned to {job.approver.name}",
            ...
            month_jobs = JORouting.objects.filter(
                approver__in=maintenance,
                status__in=['Pending', 'Approved'],
                request_at__range=[month_start, month_end]
            ).order_by('request_at')

            # Calculate weeks and track rows
            week_columns = {}
            week_rows = {}
            current_week = 1
            ...
            for job in month_jobs:
                ...
                if week_of_month not in week_columns:
                    week_columns[week_of_month] = current_week
                    week_rows[week_of_month] = 1
                    current_week += 1
...
def get_upcoming_deadlines(request):
    ...
    base_query = JOLogsheet.objects.filter(
        in_charge=current_user,
    ...
    tomorrow_count = base_query.filter(
        target_date__date=tomorrow
    ).count()
    
    this_week_count = base_query.filter(
        target_date__date__gt=tomorrow,
        target_date__date__lte=next_week
    ).count()
    
    next_week_count = base_query.filter(
        target_date__date__gt=next_week,
        target_date__date__lte=two_weeks
    ).count()
    
    later_count = base_query.filter(
        target_date__date__gt=two_weeks
    ).count()
    
    overdue_count = JORouting.objects.filter(status = "Pending", approver=request.user).filter(jo_number__target_date__lt=today).count()
